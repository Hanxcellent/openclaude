...
OpenClaude 共享同一 QueryEngine 和工具定义，但不同入口不共享同一 UI、输入协议或权限交互。理解入口差异是判断某功能是否可用的前提。
...
`src/entrypoints/mcp.ts` 由 CLI 的 MCP 子命令调用。`src/grpc/` 通过 `scripts/start-grpc.ts` 启动，只是仓库开发脚本，不在 package exports 中，不能当作稳定公开接口。
...
顺序不是任意的：provider 参数必须在认证检查和启动画面前生效；skills 本地管理要在 provider 配置损坏时仍能使用；后台 child 必须继承已经解析的 provider/model 环境。
...
工作目录信任 gate 在执行项目 hooks、plugins 和 instructions 前完成。交互模式不能把 `--print` 的“用户已经信任当前目录”假设直接复用。
...
`--bare` 设置简化模式，跳过 hooks、LSP、插件同步、attribution、auto-memory、后台预取、keychain 和自动 CLAUDE.md 发现。它不等于“没有工具”：显式 MCP、settings、agents、plugin-dir、add-dir 和 skill 仍可加载。
...
存在 `--sdk-url` 时使用 `RemoteIO`。它在 StructuredIO 之上连接远端 session transport，并维护 internal-event queue、断线恢复和远端确认。业务 query 仍在本地 headless engine 执行，但输入输出由远端控制平面传输。

Transport 实现包括 WebSocket、SSE 和 hybrid；Hybrid 可根据服务能力或连接状态选择通道，并统一向 `StdoutMessage` 事件模型转换。

## 13.6 SDK v1 Query

SDK barrel 明确不能导入 React、Ink 或 CLI/TUI 代码，构建时还检查关键模块是否被错误替换成 stub。
...
历史 CLI 代码仍使用部分全局/模块状态。SDK 用 `runWithSdkContext()` 为当前异步链提供 session ID、cwd 和 transcript dir，并对 process env 覆盖使用全程 mutex：
...
这防止同进程并发 SDK query 相互覆盖 provider credentials。它也意味着大量带不同 env override 的 query 会被串行化，不应误认为完全无共享资源。

Agent definition 或 MCP 装载失败会产生 SDK failure event/警告并继续使用剩余能力，而不是使整个 query 初始化失败。
...
长生命周期 host 必须在 `finally` 调用 `close()`，否则 abandoned session 会保留 buffer 和 pending callbacks。
...
所有传入 session ID 先校验 UUID。列表和 info 使用轻量头尾读取；messages 沿 parent UUID 选择活动链并排除 sidechain。Mutation 直接写 JSONL 元数据，因此调用者仍应考虑与正在运行会话的并发写入。
...
该入口没有对话历史、Ink 权限 UI、完整 slash command 或 agent definitions，仅保留 review command。不要假设 REPL 中所有 hook/UI 行为都会发生。
...
`useRemoteSession` 收到远端已经生成的 assistant/tool 状态时，不应再走本地工具执行；否则会造成双重副作用。SSH 断线会显示重连状态，达到最大次数后进入 graceful shutdown。

远程模式共享显示协议，不代表共享本地 filesystem、MCP 或 permission context。判断一个工具在哪里执行，应看该模式的 transport 和 QueryEngine 所在进程。

## 13.11 后台 CLI 会话

`--bg/--background` 派生新的 `--print` child，将进程身份、session ID、命令和日志位置写入 registry。`ps/logs/attach/kill` 走轻量 fast path。

kill 前会重新验证 PID/进程身份，先 SIGTERM，再按平台和存活状态升级；避免 PID 复用后杀错进程。后台 agent/task 与主会话内的 in-process subagent 不是同一机制，详见第 8 章。
...
它没有生产级持久化、认证、TLS、多租户隔离、权限超时和分布式 session store。`scripts/start-grpc.ts` 还包含开发用 MACRO polyfill。简历中可以说“仓库含 QueryEngine 的 gRPC 原型适配器”，不能说“已提供生产级 gRPC 服务”。
...
下一章从攻击面而非功能模块重新审视信任、权限、进程、网络和数据边界。
