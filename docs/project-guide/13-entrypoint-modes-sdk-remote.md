...
REPL、远程会话和 SDK 都会调用共享标题生成服务。

| 入口 | 触发时机 | 标题用途 |
|---|---|---|
| REPL | 第一条真实用户消息提交后 | 本地会话列表 |
| Remote | 没有初始 Prompt 的会话收到首条消息后 | 远程任务和会话列表 |
| SDK | generate_session_title 控制请求到达后 | 返回调用方并按配置持久化 |

共享服务负责模型调用、错误响应检查、标题解析、安全清理、默认回退和失败指标。API 错误响应不会进入任意入口的标题展示。

完整流程见 [18 简历技术专题](18-resume-technical-deep-dives.md)。
