...
因此“user”不是只代表键盘用户；模型协议要求工具结果以 user role 返回，系统注入的延续指令也可能复用 user message。
...
- tombstone 删除已经流出但随后认定无效的消息，写入层会移除对应 UUID。
...
```text
assistant: [text?, tool_use(id=A), tool_use(id=B)]
...
恢复 guard 必须跨 stop-hook retry 保留，否则会发生 compact/fallback 死循环。
...
`src/state/AppStateStore.ts` 是 vanilla store，`AppStateProvider` 把它注入 React。组件通过 selector 订阅切片，而不是每次复制整份 state。
...
### 5.1 为什么 root `setAppStateForTasks` 存在

异步子代理可以嵌套创建后台任务。父 `ToolUseContext.setAppState` 在 async agent 隔离中可能是 no-op，防止子代理改父 UI；但 task 注册和 kill 必须在根任务表可见。因此 context 额外提供 root setter，仅用于需要跨隔离边界的任务状态。
...
JSONL 每条消息有 `uuid` 和 `parentUuid`，可表示分支而不是只有数组。metadata 条目不一定参与链；恢复时：
