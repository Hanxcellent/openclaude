...
`buildTool()` 为常用字段补默认值。安全相关默认值是保守的：未知工具默认不可并发、非只读；`isDestructive` 默认 false。默认 `checkPermissions` 返回 allow，不代表跳过全局权限系统，它只表示工具没有额外的专用限制。
...
先移除 blanket deny 有两个作用：模型根本看不到禁止工具的 schema；调用阶段仍会重新鉴权，不能把 prompt 过滤当作安全边界。MCP 规则支持服务名前缀，所以可一次隐藏某个 MCP server 的全部工具。

内置工具排序后保持连续前缀，MCP 工具排在后面。这不是纯美观处理，而是为了稳定服务端 system/tool prompt cache；MCP 工具变化不应打乱内置工具的缓存前缀。
...
`isReadOnly` 和 `isConcurrencySafe` 是两个概念。只读通常适合并发，但工具可因外部状态、共享 cache 或 UI 交互声明不安全。解析失败时按不安全处理。
...
- `allow`：可包含最终输入、是否由用户修改、反馈文字和原因；
- `ask`：包含提示、建议保存的规则、blocked path、待运行 classifier；
- `deny`：必须携带拒绝原因。

显式 `deny` 优先于 allow。`alwaysAsk` 用于即使某类操作通常可放行也要求确认。规则既在工具池装配时用于 blanket 过滤，也在具体输入阶段用于内容匹配。
...
| `acceptEdits` | 自动接受常规文件编辑，不等于自动接受任意 Bash |
| `plan` | 以分析和计划为主，阻止未明确允许的写操作 |
| `dontAsk` | 不能展示询问；需要询问的操作转为拒绝 |
| `bypassPermissions` | 跳过普通交互授权，但仍不是取消所有安全边界 |
...
- `dontAsk` 不是 allow-all，它是“不得询问”，因此 ask 通常退化为 deny。
- 后台 Agent 无交互 UI，先给 `PermissionRequest` hooks 决策机会；仍为 ask 时自动拒绝。
...
输入改写后必须重新验证关键安全性质。测试专门覆盖“把 read 改成 write”“hook 在 plan mode 中放行 mutation”等场景，说明权限决定不是一次性的布尔值，而是对最终输入的约束。

## 8. 文件读写边界

文件安全主要位于 `src/utils/permissions/filesystem.ts`，核心不是简单的字符串 `startsWith(cwd)`：
...
- 复合命令拆分后逐项匹配，不能只检查第一段；
- 只剥离白名单中的安全环境变量/包装器；
- `sh/bash -c`、`sudo`、`env`、`xargs` 等不能生成过宽的持久前缀规则；
- heredoc/多行命令生成稳定但受限的建议规则；
...
`sandbox.excludedCommands` 是用户便利功能，不是安全边界。它表示某些命令不适合沙箱，应回到普通权限提示；真正的边界仍是 permission pipeline。解析失败时，显示层和执行层都采取保守选择。
...
8. prompt 中隐藏工具只是减小暴露面，执行时仍重新鉴权。
9. 沙箱关闭或不支持时，权限系统仍独立工作。
