...
此外还有 `TaskCreate/TaskUpdate/TaskList/TaskGet` 创建的**协作待办项**。它们是 Agent 团队共享的工作清单，不是 `TaskState` 中的进程任务，二者即使都叫 Task 也没有相同状态机。
...
teammate roster 是扁平的：teammate 不能再 spawn teammate，但仍可运行普通同步 subagent。in-process teammate 不能启动后台 subagent，因为子任务生命周期会超出 leader 进程中的 teammate turn。

## 4. `runAgent()` 做了什么

`src/tools/AgentTool/runAgent.ts` 复用主 `query()` 语义，但构造隔离的子上下文：
...
前台注册返回一个只创建一次的 `backgroundSignal` promise。UI 的 background-all 或自动超时解析它；父侧停止等待，但不会 abort Agent iterator。后台 closure 接管后续消费，因此“转后台”不是重启任务，也不丢已产生的上下文。
...
`Task` 多态接口目前只统一 `kill()`；spawn/render 各类型仍有专用入口。`registerTask`、`updateTaskState`、`stopTask`、output delta 和 SDK progress 由 framework 统一提供。

`isBackgroundTask()` 只把 running/pending 且已 backgrounded 的 task 放入状态栏。前台 Agent 即使已注册为 task，也不会过早显示为后台任务。
...
通知进入统一 pending notification queue，而不是直接调用父 query。下一个队列处理周期把它作为用户侧控制消息送给模型。`notified` 在状态更新时原子检查并置位，避免 TaskStop、自然完成和读取输出同时产生重复通知；后台状态变化还会取消基于旧 task state 的 speculative response。

`TaskOutput` 仍支持阻塞/非阻塞查询，但已标记 deprecated；推荐直接 Read `output_file`。阻塞读取有 timeout 和 parent abort，完成后同样置 `notified`。
...
给已停止但有 transcript 的 Agent 发消息会调用 `resumeAgentBackground()`：

1. 从 sidechain 恢复并过滤空 assistant message；
2. 重建 tool-result replacement state；
3. 恢复原 Agent definition、model 和元数据；
4. 若旧 worktree 仍存在则触碰 mtime 并继续使用，否则回退父 cwd；
5. 重新注册后台 task，执行新 prompt；
6. 完成后照常通知。

这不是恢复一条已终止的 Promise，而是用持久 transcript 开一个新的 continuation task。

## 11. Fork Agent

fork 功能开启时，省略 `subagent_type` 表示继承父对话，而不是选择 general-purpose。`buildForkedMessages()` 构造 cache-stable 前缀：
...
所有 sibling fork 的公共前缀必须字节一致，才能共享 prompt cache。fork child 虽保留 Agent schema 以维持 tool list/cache，但 `isInForkChild()` 会在调用时拒绝再次 fork，防止递归。
...
Agent 完成后：无变化的 git worktree 自动移除；有未提交/新 commit 的 worktree保留并在通知中返回路径和分支。hook-based worktree 因无法可靠判断 VCS 变化而保守保留。删除判断失败时 fail closed，不把 `0 changes` 当成已证明安全。

显式 `EnterWorktree/ExitWorktree` 管理的是主 session worktree，语义不同于某个 Agent 私有 isolation。Exit 要求用户明确 keep/remove；有变化时除非显式 `discard_changes`，否则拒绝删除。

## 13. Teammate 为什么不同于 subagent
...
完成一次 prompt 后 teammate 不转 completed，而是进入 idle loop：
...
为了防止大规模 swarm 内存增长，AppState 中 teammate 的 UI message mirror 最多保留 50 条；完整上下文仍在 runner 局部数组和 transcript 磁盘中。
...
in-process teammate 的权限询问通过 leader permission bridge 进入主 TUI 的标准确认队列。保存规则可写回共享上下文，但必须保留 leader 当前 mode，不能让 worker 的派生 mode 泄漏到协调器。
...
- 状态更新立即反映在 team UI，但不会被当成 OS 进程 kill。
...
| background permission 仍为 ask | PermissionRequest hook 后自动 deny |
