...
工具、MCP、hook 和进程异常走各自相邻的分类器，但遵循同一原则：可预期业务失败形成结构化结果；真正不可恢复的程序错误才抛到上层。
...
OpenAI-compatible 层把分类写入内部 marker，如 `openai_category=context_overflow`。用户文案会移除 marker，但 retry/query 层可稳定读取分类，不必再次猜测自然语言。
...
每次等待前会 yield `SystemAPIErrorMessage`，所以 TUI/SDK 能显示“正在重试”，而不是看似挂起。
...
API retry 只处理“同一请求是否再发一次”。若需要改变上下文、模型或 token 参数，则由 `query.ts` 通过显式 transition 重启本轮。
...
这些标志属于当前 query state，不应作为普通消息反复持久化，否则 resume 后可能错误继承一次性重试状态。
...
4. 仍失败则显示原错误并执行 StopFailure hooks。

必须按当前源码理解能力边界：`src/services/compact/reactiveCompact.ts` 在此快照中是 feature-gated stub，`isReactiveCompactEnabled()` 恒为 `false`。因此不能在简历或面试中声称当前开源构建已实际执行 reactive compact；可用路径是标准 auto compact/context collapse。

API error 后不会运行普通 Stop hooks。模型没有产生有效回答时执行 Stop hook，容易形成“API error -> hook 阻止结束 -> 同一 API error”的循环；这里仅触发 StopFailure。

### Output token 上限

这与 context overflow 不同：输入仍可被 provider 接受，只是输出被截断。

- provider 报告硬上限时，解析数值并以更低 cap 重试一次。
- feature 开启且用户没有显式设 cap 时，可先从默认 cap 升到 `ESCALATED_MAX_TOKENS`。
- 仍截断则把已产生 assistant 内容保留，追加“直接继续”的 meta user message，最多 3 次。
- 达到上限后显示原错误，不再递归续写。
...
父查询用 child AbortController 向下传播，但 child abort 不反向取消 parent。parent 监听器使用 WeakRef，child 被遗弃后仍可回收；child 主动 abort 时移除 parent listener。
...
- 对真正中断 turn 注入明确 continuation，而不是把所有以 tool result 结束的会话都当作失败。
...
在允许的 provider 路径中，stream idle/协议失败可降级到非流式请求；`CLAUDE_CODE_DISABLE_NONSTREAMING_FALLBACK` 可关闭。降级仍受一次性 guard 约束，不能在 streaming 与 non-streaming 之间反复切换。
...
| 一个 LSP server 初始化失败 | 跳过该 server，其他扩展名仍可用 |
...
SIGTERM 使用退出码 143，SIGHUP 使用 129。macOS TTY 被撤销但没有 SIGHUP 时，每 30 秒检测一次 stdin/stdout 可用性并触发清理。

`uncaughtException` 和 `unhandledRejection` 会写无 PII 诊断与事件，但 handler 本身不是业务恢复保证；核心资源仍依赖显式 cleanup registry。

## 12.13 排障顺序

1. 先识别错误来自 provider、query、tool、hook、MCP 还是 UI。
2. 查看结构化 error/category，不先依赖显示文案。
3. 确认是否已经进入一次性 transition，防止手动再套重试。
4. 检查 abort reason，而不是只看 `AbortError` 名称。
