...
空闲超时只适用于没有合法等待标记、没有有效工具期限和没有活动事件的请求。

人工交互会挂起会话空闲计时和请求总时长计时。MCP 与 TaskOutput 等长运行工具会周期性上报活性。工具期限在人工等待期间继续生效。请求恢复执行后，空闲计时从恢复时刻重新开始。

外部中止、对话框创建失败和多个权限来源竞争都需要完成一次恢复和一次资源清理。旧 generation 的回调无法修改新请求状态。

## 18. 辅助模型请求失败

会话标题等辅助模型请求使用安全默认值。辅助请求失败不会结束主会话。

模型层可能返回带 API 错误标记的 Assistant 消息。共享标题服务会在文本解析前检查该标记。错误响应进入统一失败处理，并记录失败类别和生成失败指标。

完整实现分析见 [18 简历技术专题](18-resume-technical-deep-dives.md)。
