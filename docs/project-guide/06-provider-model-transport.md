...
- **metadata**：route 定义、默认模型、标签、能力、认证提示。
- **routing**：当前 settings/env/profile/model 的目标 route。
...
profile 是当前 public env contract 的兼容桥，因此运行时保留显式 env 分支。
...
Bedrock、Vertex、Foundry 属于 Anthropic-family。它们具有独立的认证和 client 构造路径。generic OpenAI shim 不适用于这些 provider。
...
兼容接口具有供应商差异。DeepSeek、Kimi/Moonshot、Mistral、Azure、Bankr、Ollama 等包含实质例外。
...
Gemini 具有独立的 credential、content/part、function calling、thought signature 和 Vertex Gemini client 处理逻辑。该路径采用专用协议适配。
...
工具 JSON 参数可能跨多个 delta。系统必须在累积完成后执行校验。stream 中的错误应转换为 synthetic assistant API error。throw 仅用于未预期的 runtime failure。
...
这三层必须独立处理。层次混淆会造成重复 retry，或把错误模型 ID 发送到新 endpoint。
